...
class SettingsManager(private val context: Context, sharedPreferences: Lazy<SharedPreferences>) {
    ...
    // ── Aspect ratio – reactive Flows (emit on every preference change) ──────

    fun aspectRatioFlow(): kotlinx.coroutines.flow.Flow<com.swordfish.lemuroid.app.shared.settings.AspectRatio> =
        sharedPreferences
            .getString(getString(R.string.pref_key_video_aspect_ratio), "core_provided")
            .asFlow()
            .map { com.swordfish.lemuroid.app.shared.settings.AspectRatio.parse(it) }

    fun customAspectXFlow(): kotlinx.coroutines.flow.Flow<Int> =
        sharedPreferences
            .getInt(getString(R.string.pref_key_video_custom_aspect_x), 4)
            .asFlow()

    fun customAspectYFlow(): kotlinx.coroutines.flow.Flow<Int> =
        sharedPreferences
            .getInt(getString(R.string.pref_key_video_custom_aspect_y), 3)
            .asFlow()

    // ── One-shot accessors (kept for backward compat) ─────────────────────────

    suspend fun aspectRatio(): com.swordfish.lemuroid.app.shared.settings.AspectRatio =
        withContext(Dispatchers.IO) { aspectRatioFlow().first() }

    suspend fun customAspectX() = intPreference(R.string.pref_key_video_custom_aspect_x, 4)

    suspend fun customAspectY() = intPreference(R.string.pref_key_video_custom_aspect_y, 3)
