...
private fun GeneralSettings() {
    ...
    val aspectRatioValues = stringListResource(R.array.pref_video_aspect_ratio_values).toList()
    val aspectRatioState = indexPreferenceState(
        R.string.pref_key_video_aspect_ratio,
        "core_provided",
        aspectRatioValues,
    )
    val isCustomAspect = AspectRatio.parse(
        aspectRatioValues.getOrElse(aspectRatioState.value) { "core_provided" },
    ) == AspectRatio.CUSTOM
    ...
    LemuroidCardSettingsGroup(
        ...
        LemuroidSettingsList(
            ...
            items = stringListResource(R.array.pref_video_aspect_ratio_display_names),
        )
        if (isCustomAspect) {
            LemuroidSettingsSlider(
                enabled = true,
                state = intPreferenceState(
                    key = stringResource(id = R.string.pref_key_video_custom_aspect_x),
                    default = 4,
                ),
                steps = 18,
                valueRange = 1f..20f,
                title = { Text(text = stringResource(id = R.string.settings_title_custom_aspect_x)) },
            )
            LemuroidSettingsSlider(
                enabled = true,
                state = intPreferenceState(
                    key = stringResource(id = R.string.pref_key_video_custom_aspect_y),
                    default = 3,
                ),
                steps = 18,
                valueRange = 1f..20f,
                title = { Text(text = stringResource(id = R.string.settings_title_custom_aspect_y)) },
            )
        }
    }
}
