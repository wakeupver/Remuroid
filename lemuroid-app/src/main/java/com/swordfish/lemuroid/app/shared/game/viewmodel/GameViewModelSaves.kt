...
class GameViewModelSaves(
    ...
    suspend fun loadSlot(index: Int) {
        try {
            // Only the file read belongs on the IO dispatcher. loadSaveState() calls
            // retroGameView.unserializeState() which must run on the GL/emulation thread,
            // so it is intentionally NOT wrapped in withContext(Dispatchers.IO).
            val saveState = withContext(Dispatchers.IO) {
                statesManager.getSlotSave(game, systemCoreConfig.coreID, index)
            }
            saveState?.let {
    ...
    private suspend fun restoreQuickSave(saveState: SaveState) {
        ...
        if (times == 0) {
            Timber.e("restoreQuickSave: all retry attempts exhausted — auto-save could not be restored.")
        }
    }
