...
fun LemuroidSettingsList(
    ...
    val safeStateValue = state.value.coerceIn(0, items.size - 1)

    var showDialog by remember { mutableStateOf(false) }

    val safeSubtitle =
        if (safeStateValue >= 0 && useSelectedValueAsSubtitle) {
            { Text(text = items[safeStateValue]) }
        } else {
            subtitle
        }
        ...
        text = {
            Column(
                ...
                items.forEachIndexed { index, item ->
                    val isSelected by rememberUpdatedState(newValue = safeStateValue == index)
                    Row(
                        modifier =
                            Modifier
                                .fillMaxWidth()
