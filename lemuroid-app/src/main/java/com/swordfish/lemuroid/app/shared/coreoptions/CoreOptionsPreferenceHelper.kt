...
object CoreOptionsPreferenceHelper {
    ...
    fun addPreferences(
        ...
        // Curated options (hand-crafted ExposedSetting entries).
        val curatedOptions = (baseOptions + advancedOptions).filter { !it.isAutoDetected }
        if (curatedOptions.isNotEmpty()) {
            val title = context.getString(R.string.core_settings_category_preferences)
            val curatedCategory = createCategory(context, preferenceScreen, title)
            addPreferences(context, curatedCategory, curatedOptions, systemID)
        }

        // Auto-detected options — every variable reported by the core that wasn't curated.
        val autoOptions = (baseOptions + advancedOptions).filter { it.isAutoDetected }
        if (autoOptions.isNotEmpty()) {
            val title = context.getString(R.string.core_settings_category_auto_detected)
            val autoCategory = createCategory(context, preferenceScreen, title)
            addPreferences(context, autoCategory, autoOptions, systemID)
        }
    }
